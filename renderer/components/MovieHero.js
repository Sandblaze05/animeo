...
export default function MovieHero({ movieList }) {
  ...
  const handleAddToList = async (movieData) => {
    ...
    const id = movieData.id || movieData.mal_id || movieData.title;
    ...
    try {
      const exists = await isAnimeInDefaultList(movieData, profileId || undefined);
    ...
    try {
      await addAnimeToDefaultList(movieData, profileId || undefined);
    ...
    <div className="relative w-full h-[60svh] md:h-[50svh] overflow-visible">
      ...
      >
        <div className="flex h-full">
          {slidesToDisplay.map((movieData, index) => (
            <section
              key={movieData.title}
              data-index={index}
                  ...
                  <div className="flex flex-col items-center md:items-start gap-4 text-center md:text-left">
                    ...
                    <div className="gsap-content flex items-center gap-3 mt-2">
                      <motion.button
                        ...
                        onClick={() => handleAddToList(movieData)}
                        disabled={addingToList || optimisticAdded.includes(movieData.id || movieData.mal_id || movieData.title)}
                      ...
                      <motion.button
                        initial="initial" whileHover="hover" whileTap={{ scale: 0.95, y: 1 }}
                        variants={buttonVariants}
                        onClick={() => router.push(`/watch?mal_id=${movieData.id}&season=1&episode=1`)}
